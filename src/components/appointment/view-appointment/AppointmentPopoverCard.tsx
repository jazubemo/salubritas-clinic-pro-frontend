...
export default function AppointmentPopoverCard() {
  const { selectedEvent, closePopover, popoverPosition } = useCalendar();
  const { removeAppointment, isRemovingAppointment } = useAppointments();

  if (!popoverPosition || !selectedEvent) {
    return null;
  }
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0 bg-slate-50 dark:bg-slate-900 p-1 rounded-lg border border-slate-100 dark:border-slate-800">
          ...
          <button
            onClick={() => removeAppointment(selectedEvent._id)}
            title="Cancel Appointment"
            disabled={isRemovingAppointment}
